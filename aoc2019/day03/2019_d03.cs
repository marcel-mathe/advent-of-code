...
namespace Day3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Point[] points1 = route(path1arr);
            HashSet<Point> set1 = new HashSet<Point>(points1);
            ...
            Point[] points2 = route(path2arr);
            HashSet<Point> set2 = new HashSet<Point>(points2);

            IEnumerable<Point> intersects = set1.Intersect(set2);

            IEnumerable<int> distances = intersects.Select(manhattan);

            IEnumerable<int> ordered = distances.OrderBy(s => s);
            ...
            IEnumerable<int> stepCount =
                intersects.Select(p => steps(points1, p) + steps(points2, p));

            Console.WriteLine(stepCount.Min());
        }

        // steps from beginning to Point p
        static int steps(Point[] a, Point p) {
            int steps = 0;
        ...
        // convert directions to a list of points
        static Point[] route(String[] route)
        {
            List<Point> output = new List<Point>();
            int x = 0;
            int y = 0;

            foreach(String way in route)
            {
                String direction = way.Substring(0, 1);
                String l = way.Substring(1);
